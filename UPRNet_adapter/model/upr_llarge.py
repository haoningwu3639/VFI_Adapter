...
class MotionEstimator(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv_layer1 = nn.Sequential(
                nn.Conv2d(in_channels=469, out_channels=320, kernel_size=1, stride=1, padding=0),
                nn.LeakyReLU(inplace=False, negative_slope=0.1))
        self.conv_layer2 = nn.Sequential(
                nn.Conv2d(in_channels=320, out_channels=256, kernel_size=3, stride=1, padding=1),
                nn.LeakyReLU(inplace=False, negative_slope=0.1))
        self.conv_layer3 = nn.Sequential(
                nn.Conv2d(in_channels=256, out_channels=224, kernel_size=3, stride=1, padding=1),
                nn.LeakyReLU(inplace=False, negative_slope=0.1))
        self.conv_layer4 = nn.Sequential(
                nn.Conv2d(in_channels=224, out_channels=192, kernel_size=3, stride=1, padding=1),
                nn.LeakyReLU(inplace=False, negative_slope=0.1))
        self.conv_layer5 = nn.Sequential(
                nn.Conv2d(in_channels=192, out_channels=128, kernel_size=3, stride=1, padding=1),
                nn.LeakyReLU(inplace=False, negative_slope=0.1))
        self.conv_layer6 = nn.Sequential(
                nn.Conv2d(in_channels=128, out_channels=4, kernel_size=3, stride=1, padding=1))
        
        self.adapter_alpha = nn.Sequential(
                nn.Conv2d(in_channels=128, out_channels=4, kernel_size=3, stride=1, padding=1))
        self.adapter_beta = nn.Sequential(
                nn.Conv2d(in_channels=128, out_channels=4, kernel_size=3, stride=1, padding=1))
        self.adapter_alpha_conv = nn.Conv2d(in_channels=4, out_channels=4,
                    kernel_size=1, stride=1, padding=0)
        self.adapter_beta_conv = nn.Conv2d(in_channels=4, out_channels=4,
                    kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, feat0, feat1, last_feat, last_flow):
        ...
        volume = F.leaky_relu(input=corr_fn(tenFirst=feat0, tenSecond=feat1), negative_slope=0.1, inplace=False)
        ...
        # Motion Adapter
        alpha = self.adapter_alpha(feat)
        beta = self.adapter_beta(feat)
...
class SynthesisNetwork(nn.Module):
    def __init__(self):
        ...
        self.encoder_down1 = nn.Sequential(
                nn.Conv2d(in_channels=64 + 32 + 32, out_channels=128, kernel_size=3, stride=2, padding=1),
        ...
        self.encoder_down2 = nn.Sequential(
                nn.Conv2d(in_channels=128 + 64 + 64, out_channels=256, kernel_size=3, stride=2, padding=1),
        ...
        self.decoder_conv = nn.Sequential(
                ...
                nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1),
                nn.PReLU(num_parameters=64))
        self.pred = nn.Conv2d(in_channels=64, out_channels=5, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, last_i, i0, i1, c0_pyr, c1_pyr, bi_flow_pyr,
        ...
        input_feat = torch.cat((last_i, warped_img0, warped_img1, i0, i1, flow_0t_1t), 1)
        ...
        merged_img = (warped_img0 * refine_mask0 * (1 - time_period) + warped_img1 * refine_mask1 * time_period)
...
class Model(nn.Module):
    ...
    def forward(self, img0, img1, time_period,
        ...
        for level in list(range(pyr_level))[::-1]:
            ...
            bi_flows.append(F.interpolate(input=flow, scale_factor=4.0, mode="bilinear", align_corners=False))
            interp_imgs.append(interp_img)

        # directly up-sample estimated flow to full resolution with bi-linear interpolation
        bi_flow = F.interpolate(input=flow, scale_factor=4.0, mode="bilinear", align_corners=False)

        return interp_img, bi_flow, {"interp_imgs": interp_imgs, "bi_flows": bi_flows}